...
namespace AxiAuth
{
    public class api
    {
        ...
        public void login(string username, string pass)
        {
            ...
            this.response.success = response.Contains("\"success\":true");
            this.response.message = "User or password invalid!";
        }
        ...
        {
            string folder = @"Logs", file = Path.Combine(folder, "ErrorLogs.json");
        ...
        private static string req(NameValueCollection post_data)
        {
            try
            {
                using (WebClient client = new WebClient())
                {
                    ...
                    var raw_response = client.UploadValues("https://auth.axiverse.site/api/1.2/", post_data);
